...
document.querySelector('#raw-save').addEventListener('click',()=>{try{const next=JSON.parse(rawText.value);if(!Array.isArray(next)||next.some(step=>!step.act||!step.area||!step.objective||!['questline','points','stats','choice'].includes(step.type)))throw new Error();route=next.map((step,index)=>({...step,id:step.id||`custom-${Date.now()}-${index}`,bonus:step.bonus||''}));done=new Set([...done].filter(id=>route.some(step=>step.id===id)));saveRoute();rawPanel.hidden=true;renderRoute();renderEditor();}catch{alert('Use a JSON array. Each step needs act, area, objective, and type (questline, points, stats, or choice). Reward/note can be empty.');}});
